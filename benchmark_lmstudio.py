...
LONG_REQUESTS = int(os.environ.get("BENCH_LONG_REQUESTS", "12"))
LONG_CONCURRENCY = int(os.environ.get("BENCH_LONG_CONCURRENCY", "6"))
LONG_CONTEXT_TARGET_CHARS = int(os.environ.get("BENCH_LONG_CONTEXT_CHARS", "24000"))
SHORT_MAX_TOKENS = int(os.environ.get("BENCH_SHORT_MAX_TOKENS", "192"))
LONG_MAX_TOKENS = int(os.environ.get("BENCH_LONG_MAX_TOKENS", "384"))
...
- Grafana 面板标题需要中文化，并按“服务状态、压测概览、延迟、吞吐、token 消耗、GPU 资源”组织，运维判断才会更快。
- Prometheus 适合抓取最新一轮压测摘要；若需要长期趋势，可以后续把每次压测写入 Pushgateway 或 TSDB。
- 这段文本是为了构造大上下文压测语料，请在回答中综合这些信息，不要逐条复述原文。
""".strip()
...
def chat_once(model: str, prompt: str, max_tokens: int, temperature: float) -> ReqResult:
...
def summarize(results: list[ReqResult], concurrency: int, wall_time_s: float, scenario: str) -> dict[str, Any]:
    ...
    if latencies and total_completion_tokens > 0:
        per_request_tps = [
            ...
            for r in ok
            if r.completion_tokens is not None and r.latency_s > 0
        ]
        if per_request_tps:
            summary["output_tokens_per_second_avg"] = round(statistics.mean(per_request_tps), 3)
        summary["output_tokens_per_second_cluster"] = round(
            total_completion_tokens / wall_time_s if wall_time_s > 0 else 0.0,
            3,
        )
    return summary
...
def run_scenario(
    ...
    print(
        f"scenario={scenario} model={model} requests={requests} concurrency={concurrency} "
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(
                f"  request status={status} latency={result.latency_s:.2f}s "
    ...
    return {
        "summary": summary,
        "samples": [asdict(result) for result in results[: min(5, len(results))]],
    }


def run_model(model: str, long_prompt: str) -> dict[str, Any]:
    print(f"\n=== Benchmark {model} ===", flush=True)
    warmup = chat_once(model, WARMUP_PROMPT, max_tokens=64, temperature=0.0)
    print(
        f"warmup ok={warmup.ok} latency={warmup.latency_s:.2f}s "
        f"prompt_tokens={warmup.prompt_tokens} completion_tokens={warmup.completion_tokens}",
        flush=True,
    )
...
def build_long_prompt() -> str:
    context = build_long_context(LONG_CONTEXT_TARGET_CHARS)
    return (
...
def main() -> None:
    ...
    report = {
        ...
        "models": [run_model(model, long_prompt) for model in MODELS],
    }
